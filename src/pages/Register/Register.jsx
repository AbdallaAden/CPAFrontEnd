...
export default function Register() {
    ...
    const handleClick = async (e) => {
      ...
      if (passwordAgain.current.value !== password.current.value) {
        ...
        try {
          const response = await axios.post("http://localhost:8800/register", user);//may need to rewrite the path??
          //await axios.post("/controllers/registerController", user);
          console.log(response)
          history.push("/login");
        } catch (err) {
          console.log(JSON.stringify(err.response.data).replace(/"/g,""))
         alert(JSON.stringify(err.response.data).replace(/"/g,""));
        }
      }
    };
  return (
